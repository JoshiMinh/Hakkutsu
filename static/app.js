...
function renderTonariEpisodes() {
    const selected = state.tonariEpisodes.filter((item) => item.selected && item.is_public && !item.already_imported).length;
    ...
    $("#tonari-episode-list").innerHTML = state.tonariEpisodes.length ? state.tonariEpisodes.map((item) => `<label class="episode-row ${item.already_imported ? "imported" : (item.is_public ? "" : "unavailable")}">
        <input type="checkbox" data-tonari-episode="${escapeHtml(item.episode_id)}" ${item.selected ? "checked" : ""} ${item.is_public && !item.already_imported ? "" : "disabled"}>
        ...
        <span class="availability ${item.already_imported ? "imported" : (item.is_public ? "public" : "")}">${item.already_imported ? "Đã nhập" : (item.is_public ? "Có thể nhập" : "Không khả dụng")}</span>
    </label>`).join("") : '<div class="empty">Không tìm thấy chapter công khai trong feed hiện tại.</div>';
}

function updateTonariSeriesMeta() {
    const available = state.tonariEpisodes.filter((item) => item.is_public && !item.already_imported).length;
    const imported = state.tonariEpisodes.filter((item) => item.already_imported).length;
    $("#tonari-series-meta").textContent = `${available} chapter có thể nhập${imported ? ` · ${imported} đã nhập` : ""}`;
}
...
async function monitorTonariImport(jobId) {
    ...
    for (;;) {
        ...
        if (job.status === "completed") {
            state.tonariEpisodes.forEach((item) => { if (item.selected) { item.selected = false; item.already_imported = true; } });
            state.tonariJobId = null; updateTonariSeriesMeta(); renderTonariEpisodes();
            showNotice(job.current_step || `Đã import ${job.result_count || 0} chapter từ Tonari.`); await loadLibrary(); return;
        }
...
$("#tonari-results").addEventListener("click", async (event) => {
    ...
    try {
        ...
        $("#tonari-series-title").textContent = data.series.title || "Tonari manga"; updateTonariSeriesMeta(); renderTonariEpisodes();
...
$("#tonari-select-all").addEventListener("click", () => { state.tonariEpisodes.forEach((item) => { item.selected = item.is_public && !item.already_imported; }); renderTonariEpisodes(); });
$("#tonari-import").addEventListener("click", async () => {
    const episodeIds = state.tonariEpisodes.filter((item) => item.selected && item.is_public && !item.already_imported).map((item) => item.episode_id); if (!episodeIds.length || !state.tonariSeries) return;
...
document.addEventListener("click", async (event) => {
    ...
    if (approve) {
        ...
        try {
            const endpoint = `/api/chapters/${approve.dataset.approveChapter}/review`;
            const requestReview = (overrideWarnings = false) => api(endpoint, {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ approved: true, override_warnings: overrideWarnings }),
            });
            try {
                await requestReview(false);
            } catch (error) {
                if (!error.message.includes("cảnh báo QA") || !window.confirm(`${error.message}\n\nBạn đã xem kết quả và vẫn muốn xác nhận chapter để gửi sang Study?`)) throw error;
                await requestReview(true);
            }
            showNotice("Đã xác nhận chapter hoàn thành."); await loadLibrary(); await loadMangaDetail(state.selectedMangaId);
        } catch (error) { showNotice(error.message, true); }
    }
    if (publish) {
