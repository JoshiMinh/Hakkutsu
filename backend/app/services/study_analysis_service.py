...
from app.data import study_analysis_db
        ...
        with _resources_lock:
            if not _resources_ready:
                ...
                try:
                    ...
                    db_file_str = str(settings.JAMDICT_DB_PATH) if hasattr(settings, 'JAMDICT_DB_PATH') and settings.JAMDICT_DB_PATH else None
                    if db_file_str:
                        _jamdict_resource = Jamdict(db_file=db_file_str)
                    else:
                        _jamdict_resource = Jamdict()
                except Exception:
                    _jamdict_resource = None
...
def _ai_enrich(text: str, translation: str, tokens: list[dict]) -> dict:
    ...
    if settings.TRANSLATION_API_URL.startswith(("http://127.0.0.1", "http://localhost")):
        payload["reasoning_effort"] = "none"
    try:
        response = httpx.post(settings.TRANSLATION_API_URL, headers=headers, json=payload, timeout=settings.TRANSLATION_TIMEOUT)
...
def _finish_analysis(text: str, digest: str, analysis: dict) -> dict:
    ...
    study_analysis_db.save_cached_analysis(digest, text, analysis)
    return analysis
...
def analyze_sentences(pairs: list[tuple[str, str]], *, batch_size: int = 4, progress_callback=None) -> list[dict]:
    ...
    for index, (text, translation) in enumerate(pairs):
        ...
        cached = study_analysis_db.get_cached_analysis(digest)
        if cached:
