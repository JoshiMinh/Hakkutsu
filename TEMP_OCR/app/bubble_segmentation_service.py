...
def run_bubble_segmentation_job(
    ...
    try:
        ...
        if not masks:
            raise RuntimeError("MangaLens không phát hiện được bong bóng thoại trên trang này")
        ...
        with db_session() as connection:
            ...
            connection.execute(
                "UPDATE pages SET bubble_preview_path = ?, bubble_analysis_path = ?, updated_at = ? WHERE id = ?",
                (preview_relative_path, analysis_relative_path, now, page_id),
            )
            connection.execute(
