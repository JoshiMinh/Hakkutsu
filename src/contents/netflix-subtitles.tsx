...
export default function NetflixSubtitlesOverlay() {
  ...
  const handleSyncedTracks = useCallback(
    async (e: Event) => {
      ...
      let primaryTrack = options.find((t) => t.id === selectedTrackIdRef.current);
      ...
      if (!primaryTrack) {
        ...
        primaryTrack =
          options.find(
            (t) =>
              t.languageCode === srcLang ||
              t.languageCode.startsWith(`${srcLang}-`) ||
              t.languageCode.startsWith(srcLang)
          ) || options[0];

        if (primaryTrack) {
          selectedTrackIdRef.current = primaryTrack.id;
        ...
        if (primaryTrack.url) {
          ...
          document.dispatchEvent(
            new CustomEvent("hakkutsu:netflix-lazy-load-track", {
              detail: { trackId: primaryTrack.id },
            })
          );
        }
      }
  ...
  useEffect(() => {
    ...
    const checkDomSubtitles = () => {
      ...
      for (const sel of selectors) {
        ...
        if (!el) continue;
        const leafSpans = Array.from(el.querySelectorAll("span")).filter(
    ...
    checkDomSubtitles();
    const target = document.querySelector(".watch-video") || document.querySelector(".VideoContainer") || document.body;
    const observer = new MutationObserver(checkDomSubtitles);
...
const FloatingNetflixButton: React.FC<{
  ...
  useEffect(() => {
    if (settings.netflixBtnPosition && !userMovedRef.current) {
      setCurrentPos(settings.netflixBtnPosition);
    }
  }, [settings.netflixBtnPosition?.x, settings.netflixBtnPosition?.y]);
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    let latestPos = { x: startPos.x, y: startPos.y };
    ...
    const handlePointerUp = (upEvent: PointerEvent) => {
      ...
      if (dragStartRef.current) {
        ...
        if (Math.hypot(deltaX, deltaY) > 5) {
          userMovedRef.current = true;
          updateSettings({ netflixBtnPosition: latestPos });
        } else {
          onToggle();
        }
      }
